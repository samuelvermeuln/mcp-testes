...
def _load_toml_settings(
    ...
    env_config = os.getenv("GOSYSTEM_MCP_CONFIG_TOML", "").strip()
    if env_config:
...
def _resolve_context(
    ...
    resolved_context_id = (
        context_id
        or os.getenv("GOSYSTEM_CONTEXT_ID", "").strip()
        or context_settings.get("context_id")
        or ""
    )
    resolved_developer_id = (
        developer_id
        or os.getenv("GOSYSTEM_DEVELOPER_ID", "").strip()
    ...
    resolved_workspace_id = (
        workspace_id
        or os.getenv("GOSYSTEM_WORKSPACE_ID", "").strip()
    ...
    resolved_context_root = (
        context_root
        or os.getenv("GOSYSTEM_CONTEXT_ROOT", "").strip()
...
def _assets_agents_dir() -> Path:
    # src/gosystem_test_mcp/core.py -> project_root
...
def _build_auto_test_content(
    ...
    lines.append("/// File generated by gosystem-test-mcp.")
